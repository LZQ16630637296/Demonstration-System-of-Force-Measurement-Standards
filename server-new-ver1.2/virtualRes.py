...
class Communicate_Specifications:
    def __init__(self,id,up_bandwidth:str,down_bandwidth:str,latency:float,FIB_speed:str,IPsec_speed,Virtual_bandwidth:str,Firewall_Loss,network_protocol,dpdk_l3,reliability):
        self.id=id
        ...
        self.dpdk_l3=dpdk_l3
        self.reliability=reliability

class Storage_Specifications:
    def __init__(self,id,size:str,storage_bandwidth:str,IOPS:float,Backup_num:str,reliability,framework,raid,redundancy):
        self.id = id
        self.size=size
...
class Node:
    def __init__(self, uri ,id, priority:int, unit_prize,
                 ...
                 Memory_Attributes=[
                     Memory_Attributes('1','ECC'),
                     Memory_Attributes('2','非ECC'),
                     Memory_Attributes('3','RegECC')
                 ],
                 ...
                 Communicate_Specifications=[
                     Communicate_Specifications(random.randint(0,3),5 * rand, 3 * rand, rand, 1000 * rand, 10 * rand, 5 * rand, rand * 2,'TCP/IP',10 * rand,random.uniform(0.9999,0.99999)),
                     Communicate_Specifications(random.randint(0,3),5 * rand, 3 * rand, rand, 1000 * rand, 10 * rand, 5 * rand, rand * 2,'UDP',10 * rand,random.uniform(0.9999,0.99999)),
                     Communicate_Specifications(random.randint(0,3),5 * rand, 3 * rand, rand, 1000 * rand, 10 * rand, 5 * rand, rand * 2,'Http',10 * rand,random.uniform(0.9999,0.99999))
                 ],
                 Storage_Specifications=[
                     Storage_Specifications(random.randint(0,3),20 * rand, 25 * rand, rand,5,20*rand,'集中式',int(2*rand),'条带'),
                     Storage_Specifications(random.randint(0,3),20 * rand, 25 * rand, rand, 4, 30 * rand,'分布式',int(2*rand),'多副本'),
                     Storage_Specifications(random.randint(0,3),20 * rand, 25 * rand, rand, 3, 40 * rand,'集中式',int(2*rand),'纠删码')
                 ],
                 Terminal=[
                     Terminal('Mobil','PT1H30M'),
                     Terminal('Pad', 'PT45M'),
                     Terminal('PC', 'PT2H15M'),
                 ],
                 No_Terminal=[
    ...
    def getCPU_Specifications(
        ...
        if type =='Accelerated_Instruction':
            return list(CPU_Accelerated_Instruction)[0].value+','+list(CPU_Accelerated_Instruction)[1].value
    ...
    def getGPU_Specifications(
        ...
        if type =='Graphics_Memory_size':
            ...
            return self.gpu_supported_interface_names_version[0].name+','+self.gpu_supported_interface_names_version[1].name
        elif type =='Supported_Interface_Version':
            return self.gpu_supported_interface_names_version[0].version+','+self.gpu_supported_interface_names_version[1].version
        elif type =='Number_of_Stream_Processors':
            return random.randint(2000,4000)
        elif type =='ID':
            return random.randint(1,2)

    def getFPGA_Specifications(
        self, type
    )->str:
        if type =='M20K':
            return int(random.randint(10,18)*rand)
        elif type =='DSP':
            return int(random.randint(10,18)*rand)
        elif type =='LE':
            return int(random.randint(10,18)*rand)
    ...
    def getSpeed_Attributes(
        ...
        if type =='FLOPS':
            return str(35267)+','+str(67532)
        if type =='Operand_Types':
            return self.Operand_Types[0]+','+self.Operand_Types[1]
    ...
    def getCommunicate_Specifications(
        ...
        if type == 'DPDK-L3转发速率':
            return self.Communicate_Specifications[num].dpdk_l3
        if type == '可靠性':
            return self.Communicate_Specifications[num].reliability
        if type == 'ID':
            return self.Communicate_Specifications[num].id
    ...
    def getStorage_Specifications(
        ...
        if type == '存储架构':
            return self.Storage_Specifications[num].framework
        if type == 'RAID级别':
            return self.Storage_Specifications[num].raid
        if type == '冗余方式':
            return self.Storage_Specifications[num].redundancy
        if type == 'ID':
            return self.Storage_Specifications[num].id
