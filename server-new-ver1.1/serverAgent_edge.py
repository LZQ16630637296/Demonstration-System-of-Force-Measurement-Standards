...
import argparse
from virtualRes_edge import VirtualRes, Node, ResourceConfig, ComputerPower
...
parser.add_argument('--url', type=str, default='2@node')
parser.add_argument('--ID', type=str, default='e1')
...
class ServerAgent():
    def __init__(
        ...
        virtual_topics: list = [
            'mqtt/virtual/updateCPU', 
        ],
        # virtual_res:bool = False,
    ...
    def getInfo(
        ...
        if topic == 'mqtt/system/Attribute':
            msg = self.clientID + ':' + str(self.res.Node.url) + ':' + str(self.res.Node.id) + ':' + str(self.res.Node.priority)
            return msg
        if self.virtual_res:
            """TODO+ 添加更多获取资源方式"""
            if topic == 'mqtt/system/CPUFLOPS':
                msg = self.clientID + ':' + str(round(self.res.Node.config.getProcessorFLOPS('CPU'), 2)) + 'TFLOPS'
                return msg
                ...
                msg = self.clientID + ':' + str(round(self.res.Node.config.getMemoryByte('Cache'), 2)) + 'MB'
                return msg
            # TODO:消息处理（多类
            ...
            #     return msg
                ...
                str1 = str(round(self.res.Node.getComputerPower_A('Image_classification'), 2))
                str2 = str(round(self.res.Node.getComputerPower_A('Instance_segmentation'), 2))
                str3 = str(round(self.res.Node.getComputerPower_A('Image_segmentation'), 2))
                str4 = str(round(self.res.Node.getComputerPower_A('Object_detection'), 2))
                str5 = str(round(self.res.Node.getComputerPower_A('Natural_language_processing'), 2))
                str6 = str(round(self.res.Node.getComputerPower_A('Speech_recognition'), 2))
                str7 = str(round(self.res.Node.getComputerPower_A('Intelligent_recommendation'), 2))
                str8 = str(round(self.res.Node.getComputerPower_A('Reinforcement_learning'), 2))
                msg = self.clientID + ':' + str1 + ':' + str2 + ':' + str3 + ':' + str4 + ':' + str5+ ':' + str6+ ':' + str7 + ':' + str8
                return msg
                ...
                str3=str(round(self.res.Node.config.getMemoryByte_M('Cache'), 2)) + 'MB'
                msg = self.clientID + ':' + str1 + ':' + str2 + ':' + str3
                return msg

                
            elif topic == 'mqtt/system/Storage_A':
                str1=str(round(self.res.Node.config.getMemoryByte_A('Internal_storage'), 2)) + 'GB'
                str2=str(round(self.res.Node.config.getMemoryByte_A('External_storage'), 2)) + 'GB'
                str3=str(round(self.res.Node.config.getMemoryByte_A('Cache'), 2)) + 'MB'
                msg = self.clientID + ':' + str1 + ':' + str2 + ':' + str3
                return msg
...
if __name__ == '__main__':
    ...
    server.res.Node = Node(url, ID_, priority, ResourceConfig(), )
